...
# =========================
# REELS / SHORTS PREVIEW RENDERER (ADDED)
# =========================
...
def render_reel_preview(
    ...
    *,
    ...
    - Vertical 9:16 video (Reels / Shorts)
    - White background
    - Uses EXACT same cover rendering as main code
    """
    W, H = size

    # apply variant presets (same as main)
    ...
    # reuse existing cover renderer
    cover = create_shadowed_cover_clip(W, H, cfg).with_duration(duration)

    final = CompositeVideoClip([bg_clip, cover], size=(W, H))
...
if __name__ == "__main__":
    ...
    cfg = Configuration(
        ...
        cover_color_from=(0, 120, 100),      # Startfarbe für Animation (RGB)
        ...
        cover_gradient_colors =  tuple(MapColors("#AD824E", 2, background="auto")), #type:ignore Farb-Stopps für Verlauf
        ...
        shadow_opacity=210,                  # Deckkraft des Schattens | höher = dunkler, niedriger = schwächer 120
        shadow_blur=8,                      # Schattenweichzeichner | höher = weicher, niedriger = härter
        shadow_blur_margin_mult=2.5,         # Rand-Multiplikator für Schattenauslauf | höher = mehr Rand
        shadow_spread=2,                    # Ausdehnung des Schattens | höher = größer, niedriger = kompakter
        shadow_offset=(0, 2),                # Versatz des Schattens (x, y) | Werte ≠ 0 = Schatten verschoben
        ...
        shadow_alpha_gamma=1.4,              # Gamma für Schattenkante | höher = härtere Kante, niedriger = weicher
