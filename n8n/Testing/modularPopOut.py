...
class Configuration:
    ...
    y_position: int = 500
    pop_min_scale: float = 0.06   # never scale to 0 (prevents 0x0 resize errors)
...
def add_subtitles_to_video(
    ...
    for i, seg in enumerate(segments):
        ...
        # --- POP / BUBBLE-IN EFFECT ---
        pop_dur = 0.12

        def pop_scale(t):
            if t <= 0:
                return cfg.pop_min_scale
            if t >= pop_dur:
                return 1.0
            x = t / pop_dur
            c1 = 1.70158
            c3 = c1 + 1
            s = 1 + c3*(x-1)**3 + c1*(x-1)**2
            return max(cfg.pop_min_scale, float(s))

        txt_clip = txt_clip.with_effects([vfx.Resize(pop_scale)])
        # --- END POP EFFECT ---
        ...
        txt_clip = (
            txt_clip
            .with_position(("center", y))       # type: ignore
